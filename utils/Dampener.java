...
public class Dampener {
    ...
    public static double positive(double speed, double position, double begin, double end) {
        if (speed > 0)
            return Math.max(Math.min((-position / (end - begin) + (end / (end - begin))) * speed, speed), 0.0);
        else
            return speed;
    }
    ...
    public static double negative(double speed, double position, double begin, double end) {
        if (speed < 0)
            return Math.min(Math.max((-position / (end - begin) + (end / (end - begin))) * speed, speed), 0.0);
        else
            return speed;
    }
     ...
     * @return new motor speed
     */
    public static double bidirectional(double speed, double position, double beginPositive, double endPositive,
                                       double beginNegative, double endNegative) {
        return positive(negative(speed, position, beginNegative, endNegative), position, beginPositive, endPositive);
    }
}
